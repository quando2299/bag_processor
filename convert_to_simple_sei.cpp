...
int main(int argc, char** argv) {
    ...
    output.write(reinterpret_cast<char*>(start_code), 4);
    ...
    while (pos < data.size() - 4) {
        ...
        if (nal_start < data.size()) {
            ...
            if (nal_type != 6) {
                ...
                output.write(reinterpret_cast<char*>(start_code), 4);
                output.write(reinterpret_cast<char*>(&data[nal_start]), length);
            }
        }
        pos = nal_end;
    }
    ...
    output.close();
    std::cout << "Created " << output_path << " with simple SEI timestamp" << std::endl;
    return 0;
}